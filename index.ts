...
export class Clipboard {
  ...
  writeTextSync(content: string): void {
    const writeTextProcess = execFileSync(this.goClipboardPath, [ClipboardOperation.WRITE_TEXT], {
  ...
  writeImageSync(data: string | Buffer): void {
    if (data instanceof String || typeof data === "string") {
      if (process.platform === "win32") {
        ...
        const writeImageProcess = execFileSync(this.goClipboardPath, [ClipboardOperation.WRITE_IMAGE], {
          input: data,
          // TODO: consider using stdio to get stderr using pipe and handle error
          // https://nodejs.org/api/child_process.html#child_processexecfilesyncfile-args-options
          // stdio:
  ...
  listen() {
    ...
    detectPort(19559, (err, port) => {
      this?.server?.listen(port, () => {
        const addr = this.server?.address()
        if (!addr) throw new Error("Unexpected Error: TCP Socket Server not Started")
